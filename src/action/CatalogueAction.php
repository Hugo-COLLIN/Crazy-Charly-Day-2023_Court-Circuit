...
class CatalogueAction extends Action {
    ...
    public function execute(): string{
        ...
        while ($row = $rep->fetch()){
             $res.="<div class='item-produit'><a href=?action=produit&id=".$row[0]."><img class='img-produit' src='image/$row[11]'></a><a class=\"group-produit\" href=?action=produit&id=".$row[0].">".$row[1]."<br>Prix : $row[3]</a></div>";
        }
        return $res."</div></HTML>";
    }
